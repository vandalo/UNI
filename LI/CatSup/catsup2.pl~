...
%%%%%% main:
main:- numEmployees(N), main2(N), !.
%main:-  symbolicOutput(1), !, writeClauses, halt.   % print the clauses in symbolic form and halt
main2(NN):-  tell(clauses), writeClauses(NN), told,          % generate the (numeric) SAT clauses and call the solver
	...
	treatResult(Result, NN),!.

treatResult(20, N):- write('El Sat optimo es con: '), NN is N + 1, write(NN), nl, halt.
treatResult(10, N):- NN is N -1, main2(NN), shell('cat model',_),	see(model), symbolicModel(M), seen, displaySol(M), halt.
